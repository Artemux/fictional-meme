...
class SSH {
	...
	public function __construct($host,$port=22) {
		...
		$this->port = $port;
        $this->login = "vladimir";
        $this->passwd = "sc174661";
	}
    
    public function cmd ( $cmd, &$out, $disconnect = "yes" ){
        $out = "";
        try {
		    $this->connect( $this->login, $this->passwd );
		    $cycle = true;
		    while ($cycle) {
		        $data = $this->read();
		        //$out .= $data;
    		...
    		if ($disconnect == "yes") $this->disconnect();
            return 1;
	...
	private function connect_get_peer_info() {
		...
		if (!feof($this->fd)) {
			 ...
			 * and parse it
			 */
			if (!preg_match("/^SSH\-([0-9\.]+)\-([[:print:]]+)/",$init_line,$parts)) {
