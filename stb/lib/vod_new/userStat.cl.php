...
{	
	...
	public function __construct()
	{
		...
		if(isset($this->ip))
		{
			...
			if ($check!=0){
                ...
                $DB->sqlq("INSERT INTO `VOD_UserStat` VALUES ('', '".$this->ip."', '', 0, '', '', '1' )");
	...
	public function WatchNow($FilmID="", $Type = "")
	{
		if ( is_numeric($FilmID) && $Type != "npvr" ) {
            $DB = new db();
			$time=time();
            $DB->sqlq(" UPDATE `VOD_UserStat` SET `watchnow` = '".$FilmID."', `time` = '".$time."' 
                        WHERE `id` = '".$this->UserID."' LIMIT 1;");
			$DB->sqlq("UPDATE `VOD_FilmBase` SET `counter` = `counter`+1 WHERE `id` = '".$FilmID."';");
		} else {
	...
	public function UpdateWatchedFilm($FilmID="", $Type=""){
        $DB = new db();
        if ( is_numeric($FilmID) && $Type!="npvr" ){
            $num = $DB->sqlq("SELECT * FROM `VOD_WatchedFilms` WHERE `FilmID`='".$FilmID."' AND `UserID`='".$this->UserID."'")->num_rows();
            if ($num == 0){
                $DB->sqlq(" INSERT INTO `billing`.`VOD_WatchedFilms` (`id`, `UserID`, `FilmID` , `Date`)
                            VALUES ('' , '".$this->UserID."', '".$FilmID."', '".date("Y-m-d H:i:s")."');");
            }
        }
	}
	public function UpdateUserInfo($filmID)
	{
        $DB = new db();
				...
				if ($filmID){
					if ($string != NULL)
					{
						...
						while ( $i<count( $stringCheck ) ){
							if($stringCheck[$i] == $filmID){
								$temp = $stringCheck[$i];
								break;
							}	
								$i++;
						}
						if ( !isset($temp) && $filmID != '' ){
							$string .= ','.$filmID;
							...
							$this->debug[] = date("d M y H:i:s")." - Updated watched film \"".$filmID."\" for ".$this->ip;
							return true;
						}
						else
						{
