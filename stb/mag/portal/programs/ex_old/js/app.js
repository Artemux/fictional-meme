...
app.ctrls = {
    ...
    "getCatListFromHTML": function (html) {
        log('Top categories: ---');
        var cats = [],
            source_html_splited, source_html;
        ...
        for (var i = 0; i < source_html_splited.length; i++) {
			if (branch == 'video') {
				...
				var result = /href='(\/.*?)'\><b>(.*?)<\/b><\/a/igm.exec(source_html_splited[i]);
			}
            if (result == null) {
                continue;
            }
    ...
    "getSubCatListFromHTML": function (html) {
        ...
        try {
            ...
            if (branch == 'audio') {
                ...
                if (branch == 'search') {
                    try {
                        source_html = /table\swidth\=100\%\sborder=0\scellpadding=0\scellspacing=8\sclass=panel[\s\S]*?\/table/img.exec(html)[0].toString().replace(/<script.*?\/script>/igm, '');
                    } catch (e) {
        ...
        source_html_splited = source_html.split('</td>');
        for (var i = 0; i < source_html_splited.length; i++) {
            var result = /href='(\/.*?)'.*?src='(.*?)'.*?href='.*?'><b>(.*?)<\/b><\/a>/igm.exec(source_html_splited[i]);
            ...
            try {
                if (result2[1] == 'Articles' || result2[1] == 'РЎС‚Р°С‚РµР№') {
                    cat = "true";
                }
            } catch (e) {}
            cats.push({
    ...
    "setvolume": function (dir) {
        ...
        app.ctrls.volume_timer = setTimeout(function () {
...
app.views = {
    'addElement': function (obj, parent) {
        parent.appendChild(this.createHTMLFromObj(obj));
    },
    ...
    'showSection': function (layer, content_type) {
        ...
        switch (content_type) {
            case "video":
                app.ctrls.getContent(modes.emulate == false ? as.feeds['video'] : as.feeds['e' + 'video'], app.ctrls.getCatListFromHTML);
                break;
            case "audio":
                app.ctrls.getContent(modes.emulate == false ? as.feeds['audio'] : as.feeds['e' + 'audio'], app.ctrls.getCatListFromHTML);
                break;
            case "images":
                app.ctrls.getContent(modes.emulate == false ? as.feeds['images'] : as.feeds['e' + 'images'], app.ctrls.getCatListFromHTML);
                break;
            case "search":
                app.ctrls.getContent(modes.emulate == false ? active_suggest != -1 ? as.feeds['search'] + $('sug_' + active_suggest).innerHTML : as.feeds['search'] + encodeURI($('search_input').value) : as.feeds['e' + 'search'], app.ctrls.getSubCatListFromHTML);
                break;
            case "search_line":
...
app.player = {
    ...
    "interval_func": function (pos, pos_p) {
        ...
        var marginLeft = Math.floor((t_pers + 1) / 100 * app.player.cur.scrolbar_width);
        $('exPlayerStatus').style.marginLeft = marginLeft + 'px';
    ...
    "events": function (event) {
        ...
        switch (event) {
            case 1:
                ...
                clearInterval(app.player.interval_obj);
                switch (app.player.cur.playMode) {
                    case "repeat":
                        app.player._restart();
                        break;
                    case "list":
    ...
    "_jump": function (to) {
        ...
        while (app.player._jump_to_point > app.player.cur.total_time) {
            {
                app.player._jump_to_point = i <= 0.7 ? Math.round(a - app.player.intervals[app.player.cur.interval] * i) : app.player.cur.total_time;
                i += 0.1;
            }
        }
        log('app.player._jump_to: ' + app.player._jump_to);
    ...
    "create_playlist": function (object) {
        app.views.addElement(plsobj, document.body);
        var tempY = 0;
        ...
        switch (sectionName) {
            case "audio":
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (lang_info != null) {
                        ...
                        text = lang.unk + '&nbsp;("' + getIso639LangCode(data[i].lang) + '")';
                    }
                    log('audio=' + app.keypress.als_active_audio)
                    if (app.keypress.als_active_audio == -1) {
                        if (iso639[info_menu.audio].code[0] == data[i].lang[0]) app.keypress.als_index_v = i + 1;
            ...
            case "subtitles":
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (app.keypress.als_on) if (app.keypress.als_active_subt == -1) {
                        if (iso639[info_menu.subt].code[0] == data[i].lang[0]) app.keypress.als_index_v = i + 2;
