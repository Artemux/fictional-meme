...
window.onload = function () {
    ...
    stalkerClient.setMac(Api.GetDeviceMac());
    stalkerClient.init().then(function(data){

        stalkerClient.getGenres().then(function(genres){
            arrGenres = genres;
        });

        stalkerClient.getChannels().then(function(channels){
                ...
                if (settingActive == 0) {
                    if (document.getElementById("timerDiv").style.display == 'none') {
                        if (ShowMenu) {
                            ...
                            stalkerClient.getChannels(MainMenu.GenreNext()).then(function(channels){
                                MainMenu.List = channels;
                                MainMenu.FirstPage();
                                MainMenu.RestartTm();
                            });
                ...
                if (settingActive == 0) {
                    if (document.getElementById("timerDiv").style.display == 'none') {
                        if (ShowMenu) {
                            ...
                            stalkerClient.getChannels(MainMenu.GenrePrev()).then(function(channels){
                                MainMenu.List = channels;
                                MainMenu.FirstPage();
                                MainMenu.RestartTm();
                            });
