...
function Channel() {
    ...
    this.PlayNow = function () {
        ...
        var channelId = CurMenu.List[i][0];

        if (channelId == 0) {
            return false;
        }

        stalkerClient.getChannelLink(CurMenu.List[i][0]).then(
            function (stream_url) {
                if (stream_url.indexOf("http://") > -1) {

                    if (stream_url.indexOf(".m3u8") > -1) {
                        stream_url = 'auto ' + stream_url;
                    } else {
                        stream_url = 'ffrt ' + stream_url;
                    }

                } else if (stream_url.indexOf("@") > -1) {
                    //support for old version of firmware. That don`t play media with @ in link
                    stream_url = stream_url.replace(/\@/, '');
                }

                Api.PlayOttTV(stream_url)
            },
            function (error) {
                alert('error on get channel link:' + error);
            });
