...
var STB_EMULATION = false,
	...
	pictureLinks = new Array(),             //ссылки на сами большые картинки
	picturesLinks = new Array(),            //пустая переменная
	result_albumPict = new Array(),         //пустая переменная
	result_albumsTags = new Array(),        //тэги для каждого альбома по 4 на брата
	found_pictUris_fin = new Array(),       //массив имён картинок по которым далее будем получать полную картинку пример: 5560526195709097554 для http://picasaweb.google.com/userID/albumid#5560526195709097554
	found_pictSrc = new Array(),            //массив ссылок на миниатюры
	found_albumsUrls = new Array(),         //массив ссылок на страницы с полноразмерными картинками
	tempPosition = new Array();             //(x,y) координаты выделения(selection и selectionSearch) на странице картинок и результатов поиска
        var curPage = 0,                //текущая страница
            ...
            Searched_pict = 6,
            alert = 7;
	var curAlbumPage = 0,           //текущая страница с альбомами
	...
	curPictPage = 0,                //индекс текущей страницы картинок
        picturesName = new Array(),     //массив имён картинок
        ...
        pict_indicator = 0,             //вспомогательная переменная для поочерёдного использования контэйнеров для большого изображения (тэст)
...
function continueLoad(){  //определение разрешения, а в последствии языка
	...
	keyEnabled = 1;
        ...
        //loadScript(curLangIdx+'.js',fillText());
}
...
function _debug (text,outAlert){    //вывод в консоль
  if (STB_EMULATION) {
 // alert(text)
  }
  else{
...
function getElementComputedStyle(elem, prop){       //получение неустановленого явно значения свойства обьекта
...
function lowCaser(string) {  //функция преобразования в нижний регистр
    ...
    switch(curPage){
       case 0:
        ...
        case 2:
            ...
            if(slideshowDelay<4){
                document.getElementById('green').innerText = 'Slideshow delay: '+(slDelayArr[slideshowDelay]/1000)+'sec';
            }
            else{
                document.getElementById('green').innerText = 'Slideshow delay: '+(slDelayArr[slideshowDelay]/60000)+'min';
            }
            switch(win.width){
                case 1920:
        ...
        request.onreadystatechange = function ()
        {
            ...
            if (request.readyState == 4 && request.status == 200) {
				...
				posPictSelector = makeSelectionPict(0);				
                                ...
                                document.getElementById('blue').innerText = 'Fotos: '+album.pictCount[curSelectedItem + curAlbumPage*albumsAtLine];
				keyEnabled = 1;
            }
        }
        request.send(null); // send object
...
function findpictures(strp){
    ...
    if(!picture.names){
        ...
        document.getElementById('alertText').innerHTML = 'Album is empty';
        document.getElementById('alert').style.display = 'block';
        curPage = alert;
...
function vertalign(element){
    ...
    _debug(element+'   '+height);
    if (height < 135){
        document.getElementById(element).style.marginTop = 63 - height/2+'px';
    }
}

function goToPict(pictX,pictY){         //переход к конкретному изображению и проявление форм
	keyEnabled = 0;
	var coordinates = pictY*4 + pictX + curPictPage*maxPictureLines*maxPictureColum;
	tempPictNo = coordinates;
	goBigPict(coordinates);
	//_debug(' x = '+pictX+' y = '+pictY+'url = '+ pictureLinks[coordinates]);
	//document.getElementById('pictureBig_0').innerHTML =  '<img src="' + pictureLinks[coordinates] +'" alt="">';
...
//	var bigHeight = parseInt(/\d*/.exec(getElementComputedStyle('pictureBig', 'width')));
	/*  if(bigHeight > 620){
					document.getElementById('pictureBig').style.width = '620px'
					bigHeight = 620;
				}
				document.getElementById('pictureBig').style.marginTop = "30px";
				document.getElementById('pictureBig').style.marginLeft = 360 - bigHeight/2 + "px";
				//document.getElementById('pictureBig_0').style.display = 'block';
				//document.getElementById('pictures_form').style.display = 'none';
				//_debug('length of picture links array = '+pictureLinks.length);
	_debug('picture size =   '+bigHeight);
        */
...
function getSearch(searchPage){     //создание ссылки для поиска в качестве аргумента - страница (выводятся по maxPictureLines*maxPictureColum картинок за запрос)
    ...
    document.getElementById('owner').innerHTML = 'Search results for "'+url+'" ';
...
function searchReq(url){        //создание аякс запроса для получения результатов поиска
    ...
    try {
        ...
        request.onreadystatechange = function ()
        {
            if (request.readyState == 4 && request.status == 404){
                //alert('wrong name or lost internet connection');
                document.getElementById('alertText').innerHTML = 'No result for this keyword';
                document.getElementById('alert').style.display = 'block';
                curPage = alert;
                return;
            }
...
function showResult(str,viewFoto){      
    ...
    for(var i = 0;i<maxPictureLines;i++ ){
        ...
        document.getElementById('alertText').innerHTML = 'No results for this keyword';
        document.getElementById('alert').style.display = 'block';
        curPage = alert;
    }
}
                                ...
                                //if(pict_indicator){
                                 ...
                                 /*    document.getElementById('pictureBig_1').style.display = 'none';
                                   pict_indicator = 1;
                                    ...
                                    document.getElementById('pictureBig_1').innerHTML = '<img onload="resize();" id="BigPict"   src="' + searchResults[curPictS*5] +'" alt="">';
                                    document.getElementById('pictureBig_0').style.display = 'none';
                                    pict_indicator = 0;
                                }*/
                                showResult(str,1);
