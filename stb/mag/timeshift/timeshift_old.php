...
if ( $_GET['startRecord'] == 'true' && !is_array($_GET['startRecord']) ){       
        ...
        $err = $TS->StartRecord( $ts_server );
        if ( $err == 1 ){
            $txt = "process with PID ".$TS->procPID." started ";
            $DATABASE->sqlq("   INSERT INTO `TIMESHIFT_records` ( `id`, `procPID`, `processRun`, `filename`, `chanID`, `startRecord`, `getRecord`, `stopRecord` , `serverIP` ,`userIP` ,`deleted`, `comment` )
                                VALUES ('', '".$TS->procPID."', '1' ,'".$TS->db_fileName."', '".$TS->chanID."', '".$TS->date."', '', '', '".$ts_server."', '".$clientAddr."', '','тестовая запись');");
            if (DEBUG == true){
                $debug[] = $txt;
            }
            $json['filename'] = $TS->db_fileName;
...
if ( $_GET['getRecord'] == 'true' && !is_array($_GET['aboutRecord']) ){
        unset ($arr);
        $TS = new timeshift();
        $DATABASE = new db();
        $DATABASE->sqlq("   SELECT * FROM `TIMESHIFT_records` 
